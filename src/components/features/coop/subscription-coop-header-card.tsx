...
interface SubscriptionCoopHeaderCardProps {
  coop: Cooperative;
}

/**
 * Same field layout as CoopHeaderCard (Co-operatives' real detail page),
 * but reads/writes the mock coop store directly — Subscriptions hasn't
 * been cut over to the real backend, so it can't share the real-backend-only
 * CoopHeaderCard without mixing data sources. Kept as a separate small
 * component rather than adding a "which data source" branch to the real one.
 */
export function SubscriptionCoopHeaderCard({
  coop,
}: SubscriptionCoopHeaderCardProps) {
