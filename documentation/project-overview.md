...
| Super admin's top-level `/loans` (and `/savings`) page                                   | **Real** — confirmed 2026-08-24, routes into the co-op-detail drill-down above; distinct from admin/member's own `/loans`, `/savings` (still mock) | `super-admin-loans-view.tsx`, `super-admin-loans-table.tsx`              |
...
| Members Directory (`/members`, admin's own "add a member" page)                          | **Real** (2026-08-24) — list/add/edit/status all real; a member added here can log in immediately (membership ID + default `admin123`)             | `src/services/coop-member.service.ts`, `src/hooks/use-coop-members.ts`   |
